			...
			>
				<div class="py-1" role="none">
					@foreach ($years as $item)
						<a wire:navigate href="/team-stats/{{ $item->name }}" class="{{ ($year->id == $item->id) ? 'bg-gray-100 text-gray-900' : 'text-gray-700' }} text-gray-700 block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-0">{{ $item->name }} Team Stats</a>
					@endforeach
				</div>
			</div>
		</div>
