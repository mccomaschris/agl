			...
			>
				<div class="py-1" role="none">
					@foreach ($years as $item)
						<a wire:navigate href="/team-stats/{{ $item->name }}" class="{{ ($year->id == $item->id) ? 'bg-zinc-100 text-zinc-900' : 'text-zinc-700' }} text-zinc-700 block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-0">{{ $item->name }} Team Stats</a>
					@endforeach
				</div>
			</div>
		</div>
