			...
			>
				<div class="py-1" role="none">
					@foreach ($years as $item)
						<a wire:navigate href="/standings/{{ $item->name }}" class="{{ ($year->id == $item->id) ? 'bg-grey-100 text-grey-900' : 'text-grey-700' }} text-grey-700 block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-0">{{ $item->name }} Standings</a>
					@endforeach
				</div>
			</div>
		</div>
