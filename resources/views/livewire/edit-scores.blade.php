			...
			>
				<div class="py-1" role="none">
					@foreach ($weeks as $item)
						<a wire:navigate href="/scores/week/{{ $item->id }}/edit" class="{{ ($week->id == $item->id) ? 'bg-grey-100 text-grey-900' : 'text-grey-700' }} text-grey-700 block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-0">Week {{ $item->week_order }} - {{ date('F d, Y', strtotime($item->week_date)) }} Results</a>
					@endforeach
				</div>
			</div>
		</div>
