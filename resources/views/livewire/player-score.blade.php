							...
							>
								<div class="py-1" role="none">
									@foreach ($prev_seasons as $season)
										<a wire:navigate href="{{ route('player-score', ['player' => $season->player_id]) }}" class="{{ ($player->id == $season->player_id) ? 'bg-gray-100 text-gray-900' : 'text-gray-700' }} text-gray-700 block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-0">{{ $player->user->name }} {{ $season->year_name }} Season</a>
									@endforeach
								</div>
							</div>
						</div>
