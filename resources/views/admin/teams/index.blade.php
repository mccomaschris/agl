					...
					@foreach ($year->teams as $team)
						<tr class="hover:bg-grey-100 focus-within:bg-grey-lightest group text-grey-500">
							<td class="border-t px-6 py-4  group-hover:text-green-500 ">
								<a href="/admin/teams/{{ $team->id }}/edit" class="text-grey-500">{{ $team->year->name }} Team {{ $team->name }}</a>
							</td>
							<td class="border-t px-6 py-4 group-hover:text-green-500">{{ $team->year->name }}</td>
							<td class="border-t w-px">
								<a href="/admin/teams/{{ $team->id }}/edit" class="text-grey-500 group-hover:text-green-500 px-4">
