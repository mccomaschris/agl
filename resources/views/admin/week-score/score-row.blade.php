<tr data-controller="score-row" data-score-row-url="{{ route('admin.scores.update', ['score' => $score->id]) }}" data-target="score-row.container" class="{{ ($loop->index + 1) % 2 == 0  ? 'border-b-2 border-zinc-900' : '' }}">
    ...
    <td class="border-t px-2 py-4 text-center">
        <div class="relative">
            <select class="block appearance-none w-full bg-zinc-100 border border-zinc-300 text-zinc-600 p-2 rounded" name="points" data-target="score-row.pointsInput" data-action="score-row#update blur->score-row#update">
                <option value=""></option>
    ...
    <td class="border-t px-1 py-4 text-center">
        ...
        <button data-action="score-row#update" class="focus:outline-none text-zinc-400 hover:text-green-500" data-target="score-row.save" tabindex="-1">
