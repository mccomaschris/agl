						...
						<tbody class="divide-y divide-gray-200">
							@foreach ($year->playerStandings(35) as $player)
								<tr class="even:bg-white odd:bg-gray-50/50">
									<td class="py-4 pr-3 pl-4 text-sm font-medium whitespace-nowrap text-gray-900 sm:pl-6">{{ $player->points_rank }}</td>
									<td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500"><a class="font-semibold underline hover:no-underline" href="{{ route('player-score', ['player' => $player->id]) }}">{{ $player->user->name }}</a></td>
									<td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500">{{ $player->won }}</td>
									<td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500">{{ $player->lost }}</td>
									<td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500">{{ $player->tied }}</td>
									<td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500">{{ $player->points }}</td>
								</tr>
							@endforeach
						</tbody>
					</table>
