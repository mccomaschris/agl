	...
	<x-table.tbody>
		@foreach ($year->teamStandings as $team)
			<x-table.tr-body class="{{ ($loop->index + 1) % 4 == 0 ? 'border-b-2 border-zinc-900' : '' }} border-r-0! text-center!">
				...
				<x-table.td class="border-r-0!">{{ $team->won }}</x-table.td>
				<x-table.td class="border-r-0!">{{ $team->lost }}</x-table.td>
				<x-table.td class="border-r-0!">{{ $team->tied }}</x-table.td>
				<x-table.td class="border-r-0!">{{ $team->points }}</x-table.td>
